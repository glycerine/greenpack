	...
	for i := range s.Fields {
		...
		if fast {
			...
			if s.Fields[i].ZebraId < 0 {
				fmt.Fprintf(os.Stderr, "\ngreenpack error: field '%s' is missing a zid number; greenpack requires field tags `zid:\"n\"`...for each field. One can use the `addzid` utility to add these automatically to your .go source files. Or, to generate msgpack code, use the -msgp flag.\n", s.Fields[i].FieldTag)
				os.Exit(1)
			}
			// proceed
