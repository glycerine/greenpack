...
test: all
	...
	go build -o ./zebrapack2 && ./zebrapack2 -write-schema testdata/my.z -file testdata/my.go && go test -v ./testdata
	cd testdata && go generate && go test -v
	./zebrapack2 -file testdata/my.go && go test -v ./testdata/my_gen_test.go ./testdata/my.go ./testdata/my_gen.go
	./zebrapack2 -file testdata/my.go -msgp -o testdata/my_msgp_gen.go -method-prefix=MSGP -tests=false -io=false # test the -method-prefix flag
	./zebrapack2 -schema-to-go testdata/my.z > /tmp/remy.go && echo "func main() {}" >> /tmp/remy.go && go run /tmp/remy.go && rm /tmp/remy.go
